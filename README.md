...
支持在同一个 Claude Code 实例中管理多个工作区，每个工作区对应不同的项目目录。通过飞书卡片可快速切换工作区。
...
- **工作区隔离**：每个工作区对应独立的项目目录
- **自动切换**：根据消息来源自动路由到对应工作区
- **持久化存储**：工作区配置保存在 `workspace_persist.json`
- **Chat ID 绑定**：自动将飞书群聊与工作区关联
...
发送 `/切换工作区` 或点击机器人发送的工作区选择卡片进行切换。

### 工作区持久化

工作区信息自动保存到 `workspace_persist.json`，包含：
- 工作区名称和目录
- 绑定的飞书 Chat ID
- 最后活跃时间
